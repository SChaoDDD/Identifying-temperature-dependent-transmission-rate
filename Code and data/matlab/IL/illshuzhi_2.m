...
data1=readtable('ILliugan.csv');
data1=table2array(data1);
data1=data1(1:end,:);
I_il=data1(:,6);
data2=readtable('climate_il.csv');
...
low=[1,10,10,50,0];
up=[12900000,2000,3500,5000,1];
tend=580*7;
options = odeset('RelTol',1e-12,'AbsTol',1e-12);
sol=ode45(@seir,[0 tend-1],y0,options,x0);
x1=linspace(0,tend-1,586); 
y1=deval(sol,x1);
...
hold on
plot(date3,newcase(305:580),'LineWidth',1.5,'Color','b')
hold on
plot(date2,I_il(1:580),'r.', 'MarkerSize', 15)
xtickformat('yyyy')
xlabel('year');
ylabel('case');
